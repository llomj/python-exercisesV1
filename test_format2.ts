export const formatCodeSnippet = (text: string): string => {
  if (!text) return '';
  
  const isSimpleExpression = !text.includes('\n') && 
                             !/\b(def|class|if|for|while|with|try|except|finally|else|elif)\b/.test(text);
  
  if (isSimpleExpression) return text;
  if (text.includes('\n')) return text;
  
  let inString: string | null = null;
  let bracketDepth = 0;
  let currentLine = '';
  const initialLines: string[] = [];
  
  for (let i = 0; i < text.length; i++) {
    const char = text[i];
    
    if (!inString && (char === "'" || char === '"')) {
      inString = char;
      currentLine += char;
    } else if (inString === char && text[i-1] !== '\\') {
      inString = null;
      currentLine += char;
    } else if (!inString && (char === '[' || char === '(' || char === '{')) {
      bracketDepth++;
      currentLine += char;
    } else if (!inString && (char === ']' || char === ')' || char === '}')) {
      bracketDepth--;
      currentLine += char;
    } else if (!inString && bracketDepth === 0 && char === ':') {
      currentLine += char;
      initialLines.push(currentLine.trim());
      currentLine = '';
    } else if (!inString && bracketDepth === 0 && char === ';') {
      if (currentLine.trim()) initialLines.push(currentLine.trim());
      currentLine = '';
    } else {
      currentLine += char;
    }
  }
  if (currentLine.trim()) initialLines.push(currentLine.trim());

  let currentIndent = 0;
  const formattedLines: string[] = [];
  
  for (let i = 0; i < initialLines.length; i++) {
    let line = initialLines[i];
    
    // dedent before line
    if (/^(else|elif|except|finally)\b/.test(line)) {
      currentIndent = Math.max(0, currentIndent - 1);
    }
    
    // heuristic dedent for class/def
    // if we are indented but the line does not start with self. and is not a return/yield/pass
    // and looks like a print statement or top-level variable assignment
    if (currentIndent > 0) {
      if (/^(print|assert|obj\s*=|f\s*=|[a-z_]\w*\s*=\s*[A-Z])/.test(line) && !line.startsWith('self.')) {
         currentIndent = 0;
      }
      // If it's a completely new top level def or class
      if (/^(class|def|import|from)\b/.test(line)) {
         currentIndent = 0;
      }
    }
    
    formattedLines.push(' '.repeat(currentIndent * 4) + line);
    
    // indent after line
    if (line.endsWith(':')) {
      currentIndent++;
    } else if (/^(return|pass|break|continue|yield)\b/.test(line)) {
      currentIndent = Math.max(0, currentIndent - 1);
    }
  }
  
  return formattedLines.join('\n');
};
console.log(formatCodeSnippet("class MyClass: def __init__(self, x, y): self.x = x; self.y = y; obj = MyClass(1, 2)"));
console.log("----");
console.log(formatCodeSnippet("def func(x=[]): x.append(1); return x"));
console.log("----");
console.log(formatCodeSnippet("def func(): yield 1; return 'done'; g = gen(); next(g); g.send(None)"));
