#!/usr/bin/env python3
"""
Level 5 generic tail cleanup (fast mode).

Goal: Remove the exact duplicated Key Concepts marker detected by
`scripts/audit_generic_in_depth_blocks.py` in all Level 5 question files.
...
def patch_file(path: Path) -> int:
    ...
    # Rebuild via slicing so the indices remain valid.
    for m in reversed(matches):
        ...
        # Keep it TS-template safe: remove backticks if they appear in scraped text.
...
def main() -> None:
    ...
    files = [
        question_dir / "level5.ts",
        question_dir / "level5_expert_a.ts",
        question_dir / "level5_expert_b.ts",
        question_dir / "level5_intermediate_a.ts",
        question_dir / "level5_intermediate_b.ts",
    ]
    ...
    print(f"Total patched Level 5 blocks: {total}")
