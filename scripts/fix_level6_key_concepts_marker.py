#!/usr/bin/env python3
"""
Level 6 generic tail cleanup (fast mode).

Goal: Remove the exact duplicated Key Concepts marker detected by
`scripts/audit_generic_in_depth_blocks.py` in all Level 6 question files.
...
def main() -> None:
    ...
    files = [
        question_dir / "level6.ts",
        question_dir / "level6_expert_a.ts",
        question_dir / "level6_expert_b.ts",
        question_dir / "level6_intermediate_a.ts",
        question_dir / "level6_intermediate_b.ts",
    ]
    ...
    print(f"Total patched Level 6 blocks: {total}")
