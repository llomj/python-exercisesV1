...
- **Done:** **Level 10 FR 3001–3050** — `level10.ts` `level10Patterns` Q1–50 (try/except, raise, context managers, imports/`__main__`/`sys.path`/`__all__`/`__file__`, générateurs et genexprs). **Insertion** entre **3000** et **402** (les clés 3001+ n’existaient pas avant). Fragments: `scripts/fr-l10-frags/fr-l10-3001-3025.mjs`, `fr-l10-3026-3050.mjs`. Patch: `scripts/patch-fr-l10-3001-3050.mjs`. Build OK (`vite build` + `inject-precache.js`).
...
- **Done:** **Level 10 FR 3151–3200** — `level10IntermediateB` Q51–98 (3151–3198 : itertools, `json`, `pathlib`/`os.path`, `string`, …) + fin de plage **3199–3200** (`string.whitespace`, `textwrap.wrap` — vérifier banque). Fragments: `scripts/fr-l10-frags/fr-l10-3151-3175.mjs`, `fr-l10-3176-3200.mjs`. Patch: `scripts/patch-fr-l10-3151-3200.mjs` (ancre fin **3150** → **402**). Build OK (`vite build` + `inject-precache.js`).
- **Done:** **Level 10 FR 3201–3300 (reconstruction + extension)** — **Correction d’alignement** : l’insertion « 3201–3250 » avait décalé le texte de **+4** IDs (le contenu logging/`unittest`/exceptions correspondait à **3205+**). **Rebuild** : `scripts/patch-fr-l10-rebuild-3201-3300.mjs` — nouvelles entrées **3201–3204** (`textwrap.dedent`, `Template`, logging 5 niveaux, niveau par défaut) ; réaffectation des anciens blocs **3201–3246 → 3205–3250**, **3247–3250 → 3251–3254** ; nouvelles entrées **3255–3300** (PEP 8 suite, `asyncio`/idiomes, Zen, DRY). Fragments : `fr-l10-3201-3204-gap.mjs`, `fr-l10-3255-3282.mjs`, `fr-l10-3283-3300.mjs` (+ fragments historiques `fr-l10-3201-3225.mjs`, `fr-l10-3226-3250.mjs`). L’ancien `patch-fr-l10-3201-3250.mjs` reste dans l’historique Git mais est **supplanté** par le rebuild. Build OK (`vite build` + `inject-precache.js`). **Level 10 FR in-depth : 3001–3300 complet.**

### Immediate next batches
1. Optional: audits (ex. `301–600`, glossaire) ; autres niveaux si trous résiduels hors plage 3001–3300.
2. Vérifications spot banque ↔ FR après refontes.
